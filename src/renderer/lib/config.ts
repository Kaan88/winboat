import fs: typeof import("fs") = require("node:fs");
import path: typeof import("path") = require("node:path");
...
export type WinboatConfigObj = {
    ...
    // New fields (shutdown timer)
    shutdownTimer: boolean; // whether the shutdown timer feature is enabled
    timerLength: number; // timer length in milliseconds
...
const defaultConfig: WinboatConfigObj = {
    ...
    // Defaults for the new options:
    // shutdownTimer disabled by default
    shutdownTimer: false,
    // default timer length: 5 minutes in milliseconds
